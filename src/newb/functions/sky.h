...
vec3 getSunBloom(float viewDirX, vec3 horizonEdgeCol, vec3 FOG_COLOR) {
  ...
  sunBloom = 0.2*spread + sunBloom*sunBloom*sunBloom*0.8;
...
vec3 renderEndSky(vec3 horizonCol, vec3 zenithCol, vec3 viewDir, float t) {
  ...
  sky += mix(zenithCol, horizonCol, f);
  
  vec3 rg = (g*g*g*g*g*6.8)*vec3(0.3,0.0,0.6)*1.95;
  vec3 rg2 = (g2*g2*g2*g2*4.0)*vec3(0.35,0.25,0.23)*4.95;
  sky += rg + rg2;
  ...
  if (env.end) {
    ...
    if (!env.underwater) {
      ...
      if (env.underwater) {
        ...
        streaks *= streaks;
        streaks = (spread + 5.0*grad*grad + 5.0*streaks*streaks);
